...
```
atmosphere(1) → he_wait(512) → he(510) → he_end(513)
              → za_start(501) → za_wait(502) → za(500) → za_end(503) → atmosphere(1)
```

각 단계 약 30초, 전체 사이클 약 2분 15초. **R 산출에 쓰는 안정 구간은 `he`(510)·`za`(500)**이며
전이(`*_wait`)는 제외해야 한다는 기존 규약이 실데이터로 확인됐다.
...
1. **`flag=501` 미정의** — `raw_parser` 문서에도 없던 값. `he_end(513) → 501 → za_wait(502)`
   위치와 50x=ZA 계열인 점으로 보아 **ZA 시퀀스 시작 마커**로 판단해 `za_start`로 추가했다
   (단 1행만 등장. 대칭인 `511`은 관측되지 않음). R 산출에는 쓰이지 않아 이름이 틀려도
   영향은 없지만, 매핑이 없으면 대시보드에 "알 수 없음"으로 뜬다.
...
3. **캐비티 압력 밴드가 오경보** — He 주입으로 ANs 캐비티압이 **916 → 970 mbar**로 정상 상승.
   ...
   `"phases": ["atmosphere"]`로 **대기 구간에만 적용**되도록 했다. 대기 중 970 mbar는
