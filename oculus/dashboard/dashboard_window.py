"""oculus/dashboard/dashboard_window.py — 최소 실시간 대시보드 (설계문서 §6, M0+M1).

M0: 종합 상태 배지(OK/P1/P2/P0) + 파일별 최근 행 시각/지연 + append-only 로그.
M1: 파일별 HK(밴드·포화) 상태 열 추가. R/농도 패널(M2~M3)은 나중에 탭으로.
"""
...
class DashboardWindow(QMainWindow):
    def __init__(self, title: str = "Oculus — Pipeline Health"):
        ...
        self.table.setHorizontalHeaderLabels(["File", "Last row", "Lag (s)", "HK"])
    ...
    def set_status(self, status: str, msg: str) -> None:
        """종합 상태 배지 갱신(liveness + HK 등 전체 aggregate 결과)."""
    ...
    def update_files(self, rows: dict) -> None:
        """rows: {file_path: (last_row_time, lag_sec, hk_status, hk_msg)}.
        hk_status/hk_msg는 아직 HK 판정이 없으면 None으로 줘도 된다."""
        self.table.setRowCount(len(rows))
        for i, (path, info) in enumerate(sorted(rows.items())):
            last_row, lag, hk_status, hk_msg = info
            ...
            hk_item = QTableWidgetItem(f"{_BADGE_ICON.get(hk_status, '⏳')} {hk_msg or '—'}")
            color = _CELL_COLOR.get(hk_status)
            if color is not None:
                hk_item.setForeground(color)
            self.table.setItem(i, 3, hk_item)
