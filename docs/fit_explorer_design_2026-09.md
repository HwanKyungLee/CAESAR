...
없는 결과는 **“물리 검증 완료”가 아니라 “내부적으로 강건한 후보”**라고만 보고한다. 알려진 농도
인젝션은 미러·얼라인먼트·R/Leff·운전 상태가 유지된 **그 세션의** 절대 회수율을 검증할 수는 있지만,
그 계수를 다른 날짜나 장비상태에 적용하는 영구 보정으로 쓰지 않는다. 채널비가 여러 안정 plateau에서
재현된 인젝션은 `T3_SESSION_RELATIVE` 보조 증거로 기록할 수 있으며, 이는 후보의 T2 상태·순위·농도
스케일·FitSet을 바꾸지 않는다. 독립 경로/검증된 채널 간 대조도 같은 원칙으로 provenance와 범위를
명시한다. 현재 8월 인젝션은 절대 골든 정답이 아니라 session-relative 증거다.
