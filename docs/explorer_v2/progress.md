...
## 2026-09-10 재검토 정정 — 아래 과거 완료 보고를 대체함

카드 1~6은 전체 완료가 아니다. 기존 테스트 통과는 독립 함수 수준이며 실제 V2 운영 인수가 아니다.

| 카드 | 정정 상태 | 남은 조건 |
|---|---|---|
| 1 | 부분 구현 | solver 종료/종별 누락/전체 attempt identity를 내부 판정에 연결 |
| 2 | 부분 구현 | 파일 로더, 실제 runtime cfg, 입력 단위 변환, 이웃 edge, 물리 관측 분할 |
| 3 | 부분 구현 | 실제 피팅 연결, 종별 농도 단위/scale, 근거 있는 비교 기준 |
| 4 | 부분 구현 | planner edge 연결, pair 비교와 topology 분리, 미평가 edge closure |
| 5 | 미완료·추천/export 차단 | 동결 입력/후보와 실제 실행 증거 연결, holdout 사용 이력 |
| 6 | 부분 구현 | JSON 수작업 없는 입력부터 실제 fit/추천/export 운영 인수 |

이번 수정은 잘못된 추천 차단과 재현된 결함 수정 및 discovery runtime 첫 연결이다.
`run-discovery`의 실제 VARPRO 합성 데이터 2-start·재개·캐시 변조 거부를 검사했다.
실측 미션 운영 인수, 그래프/holdout/추천/GUI 통합은 아직 완료되지 않았다.
기존 V1 batch 및 사용자 파일은 유지한다. 아래 기록은 과거 보고의 이력으로만 보존한다.
