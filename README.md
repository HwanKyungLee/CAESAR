...
| **etalon** | 광학 부품의 다중 반사로 생기는 잔물결 간섭 패턴(피팅에서 보정 대상). 채널별 `Fit etalon` 체크박스 = FitSet `"etalon": {"enabled": true}`(키 없으면 켬). 끄면 FFT 검출도 sin·cos 열도 없다 — 물결이 없는데 켜 두면 좁은 창에서 NO₂ 산포가 커질 수 있다(합성 450–462 nm: 4.1 % → 8.3 %) |
