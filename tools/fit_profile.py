...
def main(argv=None):
    ...
    p.add_argument("--target", default="NO2",
                   help="target reference name; defaults to NO2 for legacy use")
    ...
    if a.target not in eng.gas_list: raise SystemExit("ABSTAIN: target is not a fitted reference")
    try: tb=__import__("core.fit_explorer",fromlist=["target_global_bounds"]).target_global_bounds(ref_props,a.target)
    ...
    def fit(sh,sq,mode):
        r=PO.fit_scan(eng,fitter,ref_props,wave,alpha,float(temp),float(pressure),c["px_min"]-px_start,c["px_max"]-px_start,c["poly"],float(cfg.get("step_limit",.5)),a.target,allow_negative_gas=policy,controlled_start=(sh,sq),controlled_bounds={f"{a.target}_sh":fit_bounds["shift"],f"{a.target}_sq":fit_bounds["squeeze"]})
        return {"shift":r["shifts"][a.target],"squeeze":r["squeezes"][a.target],"rms_sig":r["rms_sig"],"conc":r["conc"],"n_free":r["n_free"],"nonlinear_initialization":r["nonlinear_initialization"]}
    def fixed_fit(sh, sq, mode):
        ...
        r=PO.fit_scan(eng,fitter,fixed_props,wave,alpha,float(temp),float(pressure),c["px_min"]-px_start,c["px_max"]-px_start,c["poly"],float(cfg.get("step_limit",.5)),a.target,allow_negative_gas=policy,controlled_start=(float(sh),float(sq)))
        r["shift"], r["squeeze"] = float(r["shifts"][a.target]), float(r["squeezes"][a.target])
    ...
    report["candidate"]={**c,"channel_key":a.key,"target":a.target}; report["scan"]["temperature_C"]=float(temp); report["scan"]["pressure_mbar"]=float(pressure)
    tmp=a.output+".tmp"
