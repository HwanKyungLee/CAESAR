...
**정정 (2026-09-17)** — 이 도구의 첫 커밋(a52197b) 메시지는 "교차검증 결과가
`Temporal I0 Interpolation: OFF` 로 핏됐다"는 점을 개선 여지로 적었다. 그건 오독이다.
그 헤더 플래그는 `AnalysisWorker` 의 것이고, 저 결과들은 **알파 트레이스 입력**
(linear mode)이라 그 경로를 아예 타지 않는다. 알파를 만드는 `AlphaExportWorker` 는
이미 ZA 블록평균 + PCHIP 시간보간으로 I0 를 만든다 — 아래 leave-one-out 이 '가장 좋다'고
고른 전략(B)이 이미 생산 경로에 들어 있다. 여기서 잰 드리프트는 **버그가 아니라
시간당 ZA 주기가 정하는 바닥**이다. 더 줄이려면 인젝션을 더 자주 하는 수밖에 없다.
