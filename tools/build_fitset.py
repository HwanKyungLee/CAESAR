...
사용:  python tools/build_fitset.py [cold|ans|pns]
"""
import os
import sys
import json
...
CH = {
    "cold": dict(refdir="cold", wl="Calib_20260523_Hg_4line_400-497nm_Poly2.txt", label="cold"),
    "ans":  dict(refdir="roi1", wl="Calib_20260619_Hg_400-499nm_Poly2.txt", label="ANs"),
    "pns":  dict(refdir="roi2", wl="Calib_20260619_Hg_400-499nm_Poly2.txt", label="PNs"),
}
...
def main():
    ...
    c = CH[key]
    ref_dir = os.path.join(WV, c["refdir"])
    wl_path = os.path.join(ref_dir, c["wl"])

    scans, n_total = OP.gather_scans(key, 12)
    consec = OP.gather_consecutive(key, 20)

    print("#" * 92)
    print(f"# 맨바닥 FitSet 생성 — {c['label']}   입력: 웨이브칼 + {c['refdir']}/Ref_*.dat + 알파 {len(scans)}개")
    ...
    cfg, rep = FB.build_fitset(wl_path, ref_dir, scans, OP.load_wavecal,
                               ...
                               label=c["label"], progress=lambda m: print("  ·", m))
    ...
    out = os.path.join(ROOT, "scenarios", f"AutoFitSet_{c['label']}.json")
    ...
    try:
        ...
        u = next((v for v in scen["channels"].values()
                  if c["refdir"] in str(v.get("wl_path", "")).replace("\\", "/")), None)
        if u is None:
